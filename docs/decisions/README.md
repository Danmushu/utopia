...
| 0014 | [Identity from the person, scope from the token](0014-identity-from-the-person-scope-from-the-token.md) | Implemented (#180) · six read-only MCP tools plus gated `remember` over Streamable HTTP · token scope intersects base role · `query_data` remains unavailable |
| 0015 | [A recorded sentence waits for a nod](0015-recording-a-sentence-is-not-asserting-a-fact.md) | Implemented · memory facts wait in `pending_facts`, nod queue and chat card · MCP agents can propose through `remember`, with per-token provenance; only a person can confirm |
